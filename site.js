    ...
    categories.forEach((cat, idx) => {
      ...
      if (projectsInCat.length === 0) {
        ...
        html = projectsInCat.map(p => {
          ...
          const staffStatus = p.staffStatus;
          const statusClass = staffStatus ? `staff-status-${staffStatus}` : '';
          const statusLabel = staffStatus ? staffStatus.charAt(0).toUpperCase() + staffStatus.slice(1) : '';
          const isStaff = p.category === 'staff';
          return `
          <article class="project-card">
            ${img ? `<div class="project-media" style="background-image: url(${img}); background-size: cover; background-position: center;"></div>` : ''}
            <div class="project-body">
              <h3>${escapeHtml(p.title)}</h3>
              <p>${escapeHtml(p.description || '')}</p>
              <div class="project-tags">
                ${(p.tags || []).map(t => `<span class="tag">${escapeHtml(t)}</span>`).join('')}
                ${staffStatus ? `<span class="tag ${statusClass}">${statusLabel}</span>` : ''}
              </div>
              ${p.link ? `<div class="project-actions"><a class="btn" href="${escapeHtml(p.link)}">${isStaff ? 'Join Server' : 'View'}</a></div>` : ''}
            </div>
          </article>
          `;
        }).join('');
